...
class LeWinTransformerBlock_KA(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # W-MSA/SW-MSA
        attn_windows_wa = self.win_attn(x_windows_wa, mask=None)  # nW*B, window_size*window_size, C/2
        ...
        x_wa = shifted_x_wa.view(B, H * W, C//2)
...
class LeWinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # partition windows
        x_windows = window_partition(shifted_x, self.window_size)  # nW*B, window_size, window_size, C  N*C->C
...
class LeWinTransformer_Cross(nn.Module):
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        if self.shift_size > 0:
            ...
            shifted_kv = attn_kv
        # partition windows
        attn_kv_windows = window_partition(shifted_kv, self.window_size)  # nW*B, window_size, window_size, C
        attn_kv_windows = attn_kv_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C
        ...
        # W-MCA/SW-MCA
        attn_windows = self.cross_attn(x_windows, attn_kv=attn_kv_windows, mask=attn_mask)  # nW*B, window_size*window_size, C
