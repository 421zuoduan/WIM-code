...
for i in range(epoch):
    ...
    train_accuracy = total_train_right / len(cifar10_train)  # 计算训练集上的正确率
    ...
    with torch.no_grad():
        for datas in val_loader:
            ...
            output = model(data)
            val_loss = loss_fn(output, targets)  # 测试集上的loss
            total_val_loss = total_val_loss + val_loss.item()
            val_right = (output.argmax(1) == targets).sum().item()
            epoch_val_right = epoch_val_right + val_right
    # 计算该epoch的验证集上的正确率
    epoch_val_accuracy = epoch_val_right / (50000 - NUM_TRAIN)
    # 计算该Epoch内的平均Loss
    avarage_val_loss = total_val_loss / len(val_loader)
    print(f"验证集上的平均Loss:{avarage_val_loss}")
    print(f"验证集上的正确率:{epoch_val_accuracy}")
    writer.add_scalars("loss", {
        "train_Loss": train_loss.item(), "val_loss": avarage_val_loss}, i)
    writer.add_scalars("accuracy", {
        "train_accuracy": train_accuracy, "val_accuracy": epoch_val_accuracy}, i)  # 使用tensorboard绘制loss曲线和accuracy曲线
